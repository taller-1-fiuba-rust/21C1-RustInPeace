use std::{io::Write, net::{SocketAddr, TcpStream}, sync::mpsc::{self, Sender}};
...
pub fn subscribe(cmd: &[RespType], tx: &Sender<WorkerMessage>, addrs: SocketAddr, mut stream: &TcpStream) {
    ...
    for message in messages_receiver {
        ...
        println!("{}", message);
        stream.write_all(parser_service::parse_response(RespType::RBulkString(message)).as_bytes()).unwrap();
        stream.flush().unwrap();
    }    
}

...
use crate::{domain::entities::{config::Config, message::WorkerMessage}, services::commands::command_server};
use std::{net::SocketAddr, sync::{Arc, RwLock, mpsc::Sender}};

pub fn handle_command(operation: RespType, tx: &Sender<WorkerMessage>, addrs: SocketAddr, config: &Arc<RwLock<Config>>) {
                ...
                "config" => {
                    if let RespType::RBulkString(instruction) = &array[1] {
                        match instruction.as_str() {
                            "get" => {command_server::config_get(config, &array[2]).unwrap();}
                            "set" => {
